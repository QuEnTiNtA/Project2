...
def run_training_without_CV(num_epochs, lr, batch_size, device=DEVICE, weighted_sampler=False):
    ...
    # train_dir = "data/augmented_training/images"
    # train_maskdir = "data/augmented_training/groundtruth"
    train_dir = "data/train_images"
    train_maskdir = "data/train_masks"
    ...
    # if pretrain_recon:
    #     model.final_conv = nn.Conv2d(model.features[0], out_channels=3, kernel_size=1).to(DEVICE)
    #     optimizer = optim.Adam(model.parameters(), lr=lr)
    #     scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=100)
    #     dataset_pretrain = RoadDataset(
    #         image_dir=test_dir,
    #         mask_dir=test_dir,
    #         transform=transform,
    #     )
    #     pretrain_loader = DataLoader(dataset_pretrain, batch_size=batch_size//2,
    #                                  pin_memory=pin_memory, shuffle=True, num_workers=num_workers)     
    #     loss_fn_pretrain = nn.MSELoss()
    #     for epoch in range(1, num_epochs // 2 + 1):
    #         acc, f1 = train_epoch(pretrain_loader, model, optimizer, epoch, loss_fn_pretrain, scaler, pretrain_recon=True)
    #     # after pretraining, restore the final conv layer with out_channels=1
    #     model.final_conv = nn.Conv2d(model.features[0], out_channels=1, kernel_size=1).to(DEVICE)
    #     optimizer = optim.Adam(model.parameters(), lr=lr)
    #     scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=100)
